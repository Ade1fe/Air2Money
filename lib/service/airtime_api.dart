...
class AirtimeApi {
  ...
  /// Buy airtime (deducts from balance) - User pays cash for airtime
  static Future<Map<String, dynamic>> buyAirtime({
    ...
    try {
      ...
      if (res.statusCode == 200) {
        ...
        final errorBody = jsonDecode(res.body);
        throw Exception(errorBody['error'] ?? "Failed to buy airtime");
      }
    } catch (e) {
      if (e is Exception) rethrow;
      throw Exception("Network error: $e");
  ...
  /// Sell airtime (converts airtime to cash, adds to balance)
  static Future<Map<String, dynamic>> sellAirtime({
    required String userId,
    required String network,
    required String phone,
  ...
  static Map<String, dynamic> parseTransactionResponse(
    ...
    return {
      'id': apiTransaction['id'] ?? '',
      'type': apiTransaction['type'] ?? 'received',
      'amount':
          (apiTransaction['cashAmount'] ?? apiTransaction['amount'] ?? 0)
              .toDouble(),
      'date':
      ...
      'name':
          apiTransaction['network'] != null
              ? '${apiTransaction['type'] == 'sent' ? 'Airtime Purchase' : 'Airtime Conversion'} (${apiTransaction['network']})'
              : 'Unknown',
      'status': apiTransaction['status'] ?? 'pending',
      'network': apiTransaction['network'],
      'phone': apiTransaction['phone'],
